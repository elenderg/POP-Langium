...
import type { programa } from "portugues-puro-language";
import { createPortuguesPuroServices, isprograma } from "portugues-puro-language";

let services: ReturnType<typeof createPortuguesPuroServices>;
let parse:    ReturnType<typeof parseHelper<programa>>;
let document: LangiumDocument<programa> | undefined;

beforeAll(async () => {
    services = createPortuguesPuroServices(EmptyFileSystem);
    const doParse = parseHelper<programa>(services.PortuguesPuro);
...
function checkDocumentValid(document: LangiumDocument): string | undefined {
    ...
    `
        ...
        || !isprograma(document.parseResult.value) && `Root AST object is a ${document.parseResult.value.$type}, expected a 'programa'.`
        || undefined;
}
